(function () {
  function chunkRanges(documentElement) {
    return Array.from(documentElement.querySelectorAll("[data-knowledge_base-chunk]"))
  ...
  function paintTextNode(node, sourceStart, ranges) {
    ...
    boundaries.forEach(function (boundary, index) {
      ...
      span.className = "knowledge_base-chunk-highlight";
  ...
  function paintChunkBands(view) {
    ...
    var previous = view.querySelector(":scope > .knowledge_base-chunk-bands");
    if (previous) previous.remove();

    var viewRect = view.getBoundingClientRect();
    var fragments = [];
    view.querySelectorAll(".knowledge_base-chunk-highlight").forEach(function (span) {
    ...
    layer.className = "knowledge_base-chunk-bands";
    layer.setAttribute("aria-hidden", "true");

    function addBand(color, left, right, top, bottom) {
      if (right <= left || bottom <= top) return;
      var band = document.createElement("span");
      band.className = "knowledge_base-chunk-band";
  ...
  function paintVisibleBands(root) {
    root.querySelectorAll("[data-knowledge_base-rendered]:not([hidden]), [data-knowledge_base-source]:not([hidden])")
      .forEach(paintChunkBands);
  }

  function initialize(root) {
    root.querySelectorAll("[data-knowledge_base-document]:not([data-knowledge_base-ready])").forEach(function (documentElement) {
      var source = documentElement.querySelector("[data-knowledge_base-source]");
      var rendered = documentElement.querySelector("[data-knowledge_base-rendered]");
  ...
  document.addEventListener("click", function (event) {
    var button = event.target.closest("[data-knowledge_base-mode]");
    if (!button) return;
    var documentElement = button.closest("[data-knowledge_base-document]");
    var mode = button.dataset.ragMode;
    documentElement.querySelector("[data-knowledge_base-rendered]").hidden = mode !== "rendered";
    documentElement.querySelector("[data-knowledge_base-source]").hidden = mode !== "source";
    documentElement.querySelectorAll("[data-knowledge_base-mode]").forEach(function (candidate) {
