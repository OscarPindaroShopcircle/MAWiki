...
class KnowledgeBaseModel(Base, UUIDv7PrimaryKeyMixin, TimestampMixin):
    ...
    converted_source_id: Mapped[uuid.UUID | None] = mapped_column(
    ...
    source: Mapped[SourceModel] = relationship(foreign_keys=[source_id], lazy="select")
    converted_source: Mapped[SourceModel | None] = relationship(
        foreign_keys=[converted_source_id], lazy="select"
    )
