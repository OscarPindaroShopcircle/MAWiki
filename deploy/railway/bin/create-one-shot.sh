...
printf 'Created %s\nCommit and push it, then run:\n  deploy/railway/bin/provision-service.sh --project <project> --type job --name %s\n  deploy/railway/bin/run-one-shot.sh --project <project> --service job-%s\n' "$TARGET_DIR/railway.toml" "$NAME" "$NAME"
